...
def menu_item_kb(item_id: int) -> InlineKeyboardMarkup:
...
async def show_menu(message: Message):
    ...
    for item in items:
        ...
        await message.answer(text, reply_markup=menu_item_kb(item["id"]), parse_mode="HTML")
...
async def show_cart(message: Message):
    ...
    if not cart:
        await message.answer("Savatingiz bo'sh. Avval menyudan taom tanlang 🍽")
        return
    ...
    for item_id, qty in cart.items():
        ...
        total += subtotal
        lines.append(f"{item['name']} x{qty} = {subtotal:,.0f} so'm")
...
async def confirm_order(callback: CallbackQuery, state: FSMContext):
    ...
    items_payload = [{"menu_item_id": item_id, "quantity": qty} for item_id, qty in cart.items()]
    ...
    if status != 201:
        await callback.message.answer(
            f"❌ Xatolik yuz berdi: {result.get('error', 'nomalum xato')}"
        )
        await state.clear()
        return
    ...
    carts[user_id] = {}  # savatni tozalash
    ...
    await callback.message.answer(
        ...
        f"Holatni tekshirish uchun: /order_{order_id}",
        reply_markup=main_menu_kb(),
    )
    await state.clear()
...
async def my_orders(message: Message):
    ...
    for order_id in order_ids[-5:]:  # oxirgi 5 ta buyurtma
...
async def check_order_status(message: Message):
    try:
        ...
        await message.answer("Noto'g'ri buyurtma raqami.")
        return
